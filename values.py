...
def banned_bins():
    x = open('files/bannedbin.txt', 'r').readlines()
    return x


def admins():
    x = open('files/admins.txt', 'r').readlines()
    return x


def verified_gps():
    x= open('files/groups.txt', 'r').readlines()
    return x
...
def main(cc,mes,ano,cvv):
    ...
    # }
    res = requests.get("https://zdgghhhvvdds.herokuapp.com/api/" + cc)
